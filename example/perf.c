...
void initGPUTimer(GPUtimer* timer)
{
  ...
  timer->supported = 1;  //glfwExtensionSupported("GL_ARB_timer_query");
/*	if (timer->supported) {
#ifndef GL_ARB_timer_query
...
int stopGPUTimer(GPUtimer* timer, float* times, int maxTimes)
{
  ...
  while (available && timer->ret <= timer->cur) {
    ...
    if (available) {
      GLuint timeElapsed;
      glGetQueryObjectuiv(timer->queries[timer->ret % GPU_QUERY_COUNT], GL_QUERY_RESULT, &timeElapsed);
      //GLuint64 timeElapsed = 0;
      //glGetQueryObjectui64v(timer->queries[timer->ret % GPU_QUERY_COUNT], GL_QUERY_RESULT, &timeElapsed);
      timer->ret++;
      if (n < maxTimes) {
        times[n] = (float)((double)timeElapsed * 1.0E-9);
        n++;
      }
    }
  }
...
float getGraphAverage(PerfGraph* fps)
{
  ...
  for (i = 0; i < GRAPH_HISTORY_COUNT; i++) {
    avg += fps->values[i];
    if(fps->values[i] > 0) ++nvals;
  }
  return avg / nvals;  // (float)GRAPH_HISTORY_COUNT;
}
