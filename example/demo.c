...
#define nvgFontSize nvgFontHeight
...
int loadDemoData(NVGcontext* vg, DemoData* data, int imgflags)
{
  ...
  data->fontEmoji = nvgCreateFont(vg, "emoji", DATA_PATH("fonts/NotoEmoji-Regular.ttf"));
  if (data->fontEmoji == -1) {
  ...
  data->fontIcons = nvgCreateFont(vg, "icons", DATA_PATH("fonts/entypo.ttf"));
  if (data->fontIcons == -1) {
  ...
  for (i = 0; i < 12; i++) {
    char file[128];
    snprintf(file, 128, DATA_PATH("images/image%d.jpg"), i+1);
    data->images[i] = nvgCreateImage(vg, file, imgflags);
    if (data->images[i] == 0) {
