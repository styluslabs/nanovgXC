...
\n    vec2 pos = (normal_in.x > 0.0f) == (va_in.x > vb_in.x) ? va_in : vb_in;
\n
...
\n    pos.y = normal_in.y > 0.0f ? y_max : pathYMin;
\n
\n    // expand and handle special case: normal == 0 -> va is position, vb is texture coord
\n    vec2 pos_ex = (normal_in.x == 0.0f) ? va_in : (pos + 0.55f*normal_in);
\n
...
\n    //if(v0.y >= 0.5f && v1.y >= 0.5f)  // WRONG
\n    //  return width;  // *1, the height of the pixel
\n    vec2 dv = v1 - v0;
    ...
    NVGscissor* scissor, int flags, const float* bounds, const NVGpath* paths, int npaths)
{
  ...
  if (npaths == 1 && paths[0].convex && paths[0].nfill > 2) {
    ...
    for(i = 0; i < path->nfill; ++i)
      glnvg__vset2(vtx++, path->fill[i].x0, path->fill[i].y0, 0, 0, 0, 0);
  } else {
    NVGvertex* quad;
